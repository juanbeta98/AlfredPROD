    ...
    **kwargs
    ...
    new_labor_finish_time, new_labor_finish_pos, new_duration, new_distance = \
        _compute_service_end_time(
            arrival_time=arrival_time,
            start_pos=new_labor['map_start_point'],
            end_pos=new_labor['map_start_point'],
            distance_method=distance_method,
            dist_dict=dist_dict,
            vehicle_speed=vehicle_transport_speed,
            prep_time=tiempo_alistar,
...
def commit_new_labor_insertion(
    ...
    # If the existing moves_df is completely empty, just use moves_to_add
    if moves_df_updated.empty:
        moves_df_updated = moves_to_add.copy()
    # Otherwise only concat if moves_to_add has rows
    elif not moves_to_add.empty:
        moves_df_updated = pd.concat([moves_df_updated, moves_to_add], ignore_index=True)
    # else:
    #     moves_df_updated = moves_df.copy()
    ...
    for labor_id in affected_existing:
        ...
        if not move_row.empty:
            ...
            if "start_point" in move_row and "map_start_point" in labors_df_updated.columns:
                labors_df_updated.loc[
                    labors_df_updated["labor_id"] == labor_id, "map_start_point"
                ] = move_row.iloc[0]["start_point"]
            if "end_point" in move_row and "map_end_point" in labors_df_updated.columns:
                labors_df_updated.loc[
                    labors_df_updated["labor_id"] == labor_id, "map_end_point"
    ...
    new_labor_df = pd.DataFrame([new_labor_row])
    # print("\n-------LABOR ROW TYPES:", {k: type(v) for k, v in new_labor_row.items()})


    if labors_df.empty:
        labors_df_updated = new_labor_df.copy()
    else:
        labors_df_updated = labors_df[labors_df["labor_id"] != new_labor_id]
        # Only concat if the new row isn't empty
        if not new_labor_df.empty:
            labors_df_updated = pd.concat(
                [labors_df_updated, new_labor_df], ignore_index=True
            )



    # === Final consistency checks ===
    assert moves_df_updated["labor_context_id"].nunique() == len(
        moves_df_updated["labor_context_id"].unique()
    ), "Duplicate labor_context_id detected in moves_df after insertion!"
